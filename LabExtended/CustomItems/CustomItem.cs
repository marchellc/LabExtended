...
namespace LabExtended.CustomItems
{
    ...
    {
        static CustomItem()
            => TickManager.SubscribeTick(TickItems, TickOptions.NoneSeparateProfiled, "Custom Item Tick");

        private static readonly List<CustomItemInfo> _registeredTypes = new List<CustomItemInfo>();
        private static readonly LockedDictionary<ushort, CustomItem> _items = new LockedDictionary<ushort, CustomItem>();

        public static IReadOnlyList<CustomItemInfo> RegisteredItems => _registeredTypes;
        ...
        public CustomItemInfo Info { get; internal set; }
        ...
        public virtual void TickDropped() { }
        public virtual void TickInventory() { }
        public virtual void TickStatus() { }
        ...
        public static bool RegisterItem<TItem>(CustomItemInfo customItemInfo) where TItem : CustomItem
        {
            if (TryGetRegistered<TItem>(out _))
            {
                ExLoader.Warn("Custom Items", $"Tried to register a duplicate custom item: &3{typeof(TItem).FullName}&r");
                return false;
            }

            if (string.IsNullOrEmpty(customItemInfo.Id))
            {
                ExLoader.Warn("Custom Items", $"Tried to register an item with an empty ID: &3{typeof(TItem).FullName}&r");
                return false;
            }

            if (string.IsNullOrWhiteSpace(customItemInfo.Name))
            {
                ExLoader.Warn("Custom Items", $"Tried to register an item with an empty name: &3{typeof(TItem).FullName}&r");
                return false;
            }

            if (string.IsNullOrWhiteSpace(customItemInfo.Description))
            {
                ExLoader.Warn("Custom Items", $"Tried to register an item with an empty description: &3{typeof(TItem).FullName}");
                return false;
            }
        ...
        public static bool TryGetRegistered<TItem>(out CustomItemInfo customItemInfo) where TItem : CustomItem
            ...
            foreach (var pair in _items)
            {
                try
                {
                    if (pair.Value.IsDropped)
                        pair.Value.TickDropped();
                    else if (pair.Value.IsInInventory)
                        pair.Value.TickInventory();
                    else
                        pair.Value.TickStatus();
                }
                catch (Exception ex)
                {
        ...
        internal static void InternalHandleWaiting(WaitingForPlayersEvent ev)
            => _items.Clear();
    }
}