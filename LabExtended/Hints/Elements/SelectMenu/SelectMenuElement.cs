...
using LabExtended.Hints.Elements.SelectMenu.Listeners.Keybind;
    ...
    public class SelectMenuElement : HintElement
    {
        ...
        public override void Write()
        {
            foreach (var option in _options)
            {
                ...
                Builder.Append($"<b>{(CanSelect && CurrentOption != null && CurrentOption == option ? "<color=#00ffec>→</color> " : "")}<color=#d4ff33>[{option.Position}]</color> | <color={(!IsSelected(option) ? "#ff0000" : "#4dff00")}>{option.Label}</color></b>\n");
            }
        }
